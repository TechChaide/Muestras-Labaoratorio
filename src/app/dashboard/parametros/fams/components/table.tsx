...
interface FamiliasTableProps {
  records: Familia[];
  isLoading: boolean;
  onEdit: (record: Familia) => void;
  onAddNew: () => void;
}

export default function FamiliasTable({ records, isLoading, onEdit, onAddNew }: FamiliasTableProps) {
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(PAGE_SIZE_OPTIONS[0]);
  const [filter, setFilter] = useState("");

  // Filtrado por nombre o mnemónico
  const filteredRecords = useMemo(() => {
    if (!filter.trim()) return records;
    const f = filter.toLowerCase();
    return records.filter(r =>
      (r.nombre_familia?.toLowerCase().includes(f) || "") ||
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
            ...
            <CardDescription>Familias registradas.</CardDescription>
        </div>
      ...
      <CardContent>
        ...
        <div className="flex justify-between items-center mb-4">
          <input
            ...
            placeholder="Filtrar por nombre de familia..."
            value={filter}
            onChange={e => {
        ...
        <div className="border rounded-md overflow-hidden">
            <Table>
                ...
                {isLoading ? renderSkeleton() : paginatedRecords.map((record) => (
                    <TableRow key={record.codigo_familia}>
